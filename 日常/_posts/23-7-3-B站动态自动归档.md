...
div=(a)=>tag('div',a)
pre=(a)=>tag('pre',a)
template=([file,content])=>div(file)+textarea(content)
a=files.map(template).join('<hr/>')
a=rewrite(a)

a.querySelectorAll('textarea').forEach(a=>{a.style.height="";a.style.height=a.scrollHeight+"px"})
a.querySelectorAll('textarea').forEach(a=>{a.onfocus=(()=>console.log(a.value))})
```
