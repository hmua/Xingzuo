...
a=groupBy(a,([_,d])=>jekyllDate(d.getFullYear(),d.getMonth(),d.getDate()))
b=[];for await(i of Object.keys(a))b.push([i,a[i]])
files=b.map(([d,items])=>[d,items.map(([a,_])=>a).join('\n\n')])
...
rewrite=html=>{document.write(html);document.close();return document}
s='style=\'width:100%\''
textarea=(a)=>`<textarea ${s}>${a}</textarea>`
input=(a)=>`<input value=\'${a}\'></input>`
tag=(name,a)=>`<${name}>${a}</${name}>`
div=(a)=>tag('div',a)
pre=(a)=>tag('pre',a)
template=([file,content])=>textarea(file+'-.yml')+textarea(content)
a=files.map(template).join('<hr/>')
a=rewrite(a)
copy=t=>{navigator.clipboard.writeText(t).then(()=>new Notification('✓ '+t),()=>new Notification('× '+t))}
a.querySelectorAll('textarea').forEach(a=>{
	a.readOnly=true;a.rows=1;a.style.height="";a.style.height=a.scrollHeight+"px"
