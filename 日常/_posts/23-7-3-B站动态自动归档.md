...
parse=([items,offset])=>[items.map(([a,time])=>[new Date(time*1000).toLocaleString(),a]),offset]
async function*load(offset=''){
	a=await loadCache(nextPg(offset)).catch(()=>'return')
	if(a=='return')return
	// a=await loadCache(nextPg(offset))
	a=extract(a)
	var[a,offset]=parse(a)
	yield await a
	yield*load(offset)
}
a=load()
