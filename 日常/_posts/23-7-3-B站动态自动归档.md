...
formatFiles=async a=>{
	...
	fileName=d=>`${d.getFullYear()-2000}-${d.getMonth()+1}-${d.getDate()}-.md`
	post=a=>`---\ntweets: |-\n  ${a.replaceAll('\n','\n  ')}\n---\n{{page.tweets}}`
	a=groupBy(a,([_,d])=>fileName(new Date(d*1000)))
	b=[];for await(i of Object.keys(a))b.push([i,a[i]])
	a=b.map(([d,items])=>[d,items.map(([a,_])=>a).join('\n\n')])
	return a.map(([a,b])=>[a,post(b)])
}
//生成界面，按每天复制文件名和内容
build=a=>{
...
a=loadRaw();a=await formatFiles(a)
build(a)
```
{%endraw%}
