...
formatFiles=async a=>{
	...
	name=d=>`${d.getFullYear()-2000}-${d.getMonth()+1}-${d.getDate()}-.md`
	file=a=>`---\ntweets: |-\n  ${a.replaceAll('\n','\n  ')}\n---\n{{page.tweets}}`
	a=JSON.parse(a)
...
var[a,old]=[await formatFiles(a),(await formatFiles(old)).map(([n,_])=>n)]
for(a of a)console.log(a,old.includes(a[0]))
```
{%endraw%}
