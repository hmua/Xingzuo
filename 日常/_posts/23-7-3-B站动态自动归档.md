...
{%raw%}
```js
loadRaw=()=>JSON.parse(localStorage[localStorage.key(0)])
parse=([a,d])=>[a,new Date(d*1000)]
a=loadRaw();a=a.map(parse)
groupBy=(x,f)=>x.reduce((a,b,i)=>((a[f(b,i,x)]||=[]).push(b),a),{})
jekyllDate=((y,m,d)=>`${y}-${m}-${d}`)
a=groupBy(a,([_,d])=>jekyllDate(d.getFullYear()-2000,d.getMonth()+1,d.getDate()))
...
var[a,old]=[JSON.parse(a),JSON.parse(old).map(([_,ts])=>ts)]
for(a of a)console.log(a,old.includes(a[1]))
```
