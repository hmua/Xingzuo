...
extract=(a)=>
	[a.items.map(a=>{
		...
		console.assert(b.text==b.b&&b.b==b.c,b)
		return[b.text,a.modules.module_author.pub_ts]
	}),a.offset]
nextPg=offset=>window.location.href.replace('space?offset=&',`space?offset=${offset}&`)
parse=([items,offset])=>[items.map(([a,time])=>[new Date(time*1000).toLocaleString(),a]),nextPg(offset)]
a=window.location.href
a=await loadCache(a)
a=extract(a)
parse(a)
