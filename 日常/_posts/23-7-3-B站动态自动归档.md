...
```js
a='https://api.bilibili.com/x/polymer/web-dynamic/v1/feed/space?offset=&host_mid=28748982&timezone_offset=-480&features=itemOpusStyle'
fetch(a,{credentials:'include',cache: "only-if-cached"}).then(response => response.json()).then(users => console.log(users)).catch(error => console.error(error))
```
